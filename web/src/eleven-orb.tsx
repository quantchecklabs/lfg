...
export type AgentState = null | "thinking" | "listening" | "talking" | "consulting"
...
function Scene({
  ...
  const { gl } = useThree()
  // World-space size of the canvas at z=0. Used to scale the orb so its disc
  // always fits inside the *shorter* dimension with margin — independent of the
  // container's aspect ratio — so it can never be clipped to a flat chord.
  const viewport = useThree((s) => s.viewport)
  const fitScale = (Math.min(viewport.width, viewport.height) / 2 / 3.5) * 0.9
  const circleRef =
  ...
  useFrame((_, delta: number) => {
    ...
    if (modeRef.current === "manual") {
      ...
      } else if (agentRef.current === "consulting") {
        // Deep-think: a slow, large-amplitude swell that visibly morphs the orb
        // (distinct from the quick shimmer of plain "thinking").
        const swell = 0.5 + 0.45 * Math.sin(t * 0.55)
        targetIn = clamp01(swell)
        targetOut = clamp01(0.85 - 0.45 * Math.sin(t * 0.55))
      } else {
  ...
  return (
    <mesh ref={circleRef} scale={fitScale}>
...
void main() {
    ...
    // Feather the outer edge to fully transparent so the orb's silhouette is a
    // soft alpha falloff rather than the circle geometry's hard boundary. This
    // removes the visible "weird edge", and means any clipping at the canvas
    // border lands in the already-transparent halo instead of slicing a flat
    // chord across a solid disc.
    color.a *= 1.0 - smoothstep(0.86, 1.0, radius);
